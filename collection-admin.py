...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--add", default="", help="Collection slug to add to the list")
    parser.add_argument("--remove", default="", help="Collection slug to remove from the list")
    ...
    if args.add:
        res = query(conn, f"SELECT collection_slug FROM collections WHERE collection_slug='{args.add}'")
        if len(res) == 0:
            query(conn, f"INSERT INTO collections (collection_slug) VALUES ('{args.add}')")
            print(f"Added collection '{args.add}'")
        else:
            print(f"Collection '{args.add}' already on the list")

    if args.remove:
        res = query(conn, f"SELECT collection_slug FROM collections WHERE collection_slug='{args.remove}'")
        if len(res) != 0:
            query(conn, f"DELETE FROM collections WHERE collection_slug='{args.remove}'")
